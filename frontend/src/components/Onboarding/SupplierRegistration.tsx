...
const supplierDocuments: DocumentRequirement[] = [
  { id: "businessLicense", label: "Trade / Business Licence", required: true, description: "Issued by Ethiopian trade bureau. Must be current and not expired." },
  { id: "tinCertificate", label: "TIN Registration Certificate", required: true, description: "ERCA-issued TIN certificate. 11-digit TIN validated against ERCA API." },
  { id: "auditedFinancials", label: "Audited Financial Statements", required: true, description: "Last 2 years minimum. Required for Tier 1 & 2 classification." },
  { id: "bankStatement", label: "Bank Statement (6–12 months)", required: true, description: "Used to assess cash-flow score in the 5-factor risk model." },
  { id: "memorandumArticles", label: "Memorandum & Articles of Association", required: true, description: "Companies only. For PLC, Share Company, or LLC entities." },
  { id: "beneficialOwnership", label: "Beneficial Ownership Declaration", required: true, description: "NBE AML/CFT requirement." },
  { id: "nationalId", label: "National ID / Passport of Signatories", required: true, description: "Managers and authorised signatories." },
  { id: "boardResolution", label: "Board Resolution/Authorisation Letter", required: true, description: "Companies only. Authorises individual to sign SCF agreements." },
  { id: "proofOfAddress", label: "Proof of Address", required: true, description: "Utility bill, lease agreement dated within 3 months." },
  { id: "bankConfirmation", label: "Bank Account Confirmation Letter", required: true, description: "Official bank letter confirming account number." },
  { id: "sampleInvoice", label: "Sample ERCA-compliant Invoice", required: true, description: "At least one live ERCA invoice as proof of QR-code compliance." },
  { id: "vatCertificate", label: "VAT Registration Certificate", required: false, description: "Required if supplier is VAT-registered." },
  { id: "taxClearance", label: "Tax Clearance Certificate", required: false, description: "Required for Tier 1 classification.", conditional: "Confirms no outstanding tax obligations" },
  { id: "sectorLicence", label: "Trade / Sector Licence", required: false, description: "E.g., construction grading certificate, import/export licence.", conditional: "If applicable to sector" },
]
...
export function SupplierRegistration() {
  ...
  const isRequiredDocumentsUploaded = () => {
    const requiredDocs = supplierDocuments.filter(doc => doc.required)
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (allUploadsSuccessful) {
        alert('Supplier registration submitted successfully! You will be notified once approved.')
    ...
    <div className="min-h-screen bg-background">
      <div className="container mx-auto py-8 px-4 md:px-8">
        <Card className="w-full">
          <CardHeader>
            <CardTitle className="text-3xl font-bold">Supplier Registration</CardTitle>
            <CardDescription className="text-base">
              Complete all required information to register as a supplier for Supply Chain Financing
            </CardDescription>
          </CardHeader>
          <CardContent>
            {/* Progress Steps */}
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <Input placeholder="supplier@company.com" type="email" {...field} />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <Input value="Supplier" disabled className="bg-muted" />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <Card>
                        ...
                        <CardContent className="space-y-2">
                          ...
                          <p className="text-sm"><strong>Entity Type:</strong> Supplier</p>
                        </CardContent>
                      </Card>
                          ...
                          <div className="space-y-1 max-h-96 overflow-y-auto">
                            {Object.keys(uploadedFiles).length === 0 ? (
                              ...
                              Object.entries(uploadedFiles).map(([id, file]) => {
                                const doc = supplierDocuments.find(d => d.id === id)
                                return (
